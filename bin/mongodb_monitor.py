...
for mongodb_ins in mongodb_items:
        ...
        err,conn = mongodb_monitor.mongodb_connect(host=mongodb_ins["server"],port=mongodb_ins["port"], user=mongodb_ins["user"], password=mongodb_ins["password"])
 
	mongodb_upate_list = [] 
        if err != 0:
		key_item_dict =  {"endpoint": endpoint, "metric": "mongo_local_alive", "tags":mongodb_tag , "timestamp":ts, "value": 0, "step": 60, "counterType": "GAUGE"}
        ...
        for mongodb_metric in mongodb_dict_keys:
                ...
                if mongodb_metric in mongodb_counter_metric :
                        key_item_dict = {"endpoint": endpoint, "metric": mongodb_metric, "tags":mongodb_tag , "timestamp":ts, "value": mongodb_dict[mongodb_metric], "step": 60, "counterType": "COUNTER"}
                else:
                        key_item_dict =  {"endpoint": endpoint, "metric": mongodb_metric, "tags":mongodb_tag , "timestamp":ts, "value": mongodb_dict[mongodb_metric], "step": 60, "counterType": "GAUGE"}
